...
private:
    double radians;
    double convertToDegrees(double radians) { return normalize(radians) * 360.0 / (2 * M_PI); }
    double convertToRadians(double degrees) {return normalize(degrees * (2 * M_PI) / 360.0); }
    double normalize(double radians) { return radians - floor(radians / (2 * M_PI)) * (2 * M_PI); }
};
...
private:
    void testConstructor() {
        ...
        tester = Angle(120);
        assert(tester.radians > 2.0943 && tester.radians < 2.0945);
        
        tester = Angle(430);
        assert(tester.radians > 1.22172 && tester.radians < 1.22174);
        
        tester = Angle(0);
        assert(tester.radians == 0);
        
        
        tester = Angle(-120);
        assert(tester.radians > 4.18878 && tester.radians < 4.18880);
    ...
    void testGetDegrees() {
        ...
        assert(tester.getDegrees() > 212.957 && tester.getDegrees() < 212.959);
        
        tester.radians = -15.0;

        assert(tester.getDegrees() > 220.562 && tester.getDegrees() < 220.564);
        
        tester.radians = 2.0;
        
        assert(tester.getDegrees() > 114.591 && tester.getDegrees() < 114.593);
        
        tester.radians = 3.7;
        
        assert(tester.getDegrees() > 211.993 && tester.getDegrees() < 211.995);
        
        tester.radians = 4.3;
        assert(tester.getDegrees() > 246.371 && tester.getDegrees() < 246.373) ;
    ...
    void testSetDegrees() {
        ...
        assert(tester.radians > 0.0872664 && tester.radians < 0.0872666);
        ...
        assert(tester.radians > 4.10151 && tester.radians < 4.10153);
        assert(tester.getDegrees() > 234.9 && tester.getDegrees() < 235.1);
        
        tester.setDegrees(290.3);
        assert(tester.radians > 5.06668 && tester.radians < 5.0667);
        assert(tester.getDegrees() == 290.3);
        
        tester.setDegrees(39.25);
        assert(tester.radians > 0.685041 && tester.radians < 0.685043);
        assert(tester.getDegrees() > 39.24 && tester.getDegrees() < 39.26);
        
        tester.setDegrees(-212.3);
        assert(tester.radians > 2.57784 && tester.radians < 2.57786);
        assert(tester.getDegrees() == 147.7);
    ...
    void testSetRadians() {
        ...
        assert(tester.radians > 3.54955 && tester.radians < 3.54957);
        ...
        assert(tester.radians > 4.78318 && tester.radians < 4.7832);
        
        
        tester.setRadians(-2);
        assert(tester.radians > 4.28318 && tester.radians < 4.2832);
    ...
    void testNormalize() {
        ...
        assert(tester.normalize(15.0) > 2.43362 && tester.normalize(15.0) < 2.43364);
        assert(tester.normalize(-3.142) > 3.14118 && tester.normalize(-3.142) < 3.1412);
        assert(tester.normalize(-5.2) > 1.08318 && tester.normalize(-5.2) < 1.0832);
...
public:
    void run() {
        PhysicsTester t = PhysicsTester();
        t.run();
        AngleTester tester2 = AngleTester();
        tester2.run();
